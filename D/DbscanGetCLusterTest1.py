...
def DBSCAN_Conbine(IPs, eps=1.0, min_samples=10):
    ...
    sql1 = "ALTER TABLE dbscan_ip_cluster_traindata ADD `{}` varchar(50)".format(column_name)
    cursor.execute(sql1)
    db.commit()
    for IP in IPs:
        sql = "SELECT * FROM traindata where ip='%s'" % IP[0]
        cursor.execute(sql)
        ...
        sql2 = "UPDATE dbscan_ip_cluster_traindata SET `{}` = %s WHERE ip = %s".format(column_name)
        values = (count, IP[0])
...
if __name__ == '__main__':
    ...
    db = mysqlconnect()
    cursor = db.cursor()
    sql1 = "SELECT ip FROM traindata"
    cursor.execute(sql1)
    results = cursor.fetchall()
    for result in results:
        ip = result[0]
        sql2 = "INSERT INTO dbscan_ip_cluster_traindata (ip) VALUES (%s)"
        values = (ip)
        cursor.execute(sql2, values)
    db.commit()
